...
/** gap = startNext - endCurrent; bloco só se gap > 30 min (estrito). */
function shouldShowFreeBlock(gapMs: number): boolean {
  return gapMs > MIN_FREE_MS;
}
...
/**
 * Monta itens da timeline mobile em ordem cronológica (evento, livre, agora, …).
 */
export function buildMobileDayTimelineItems(
  ...
  const isToday =
    ...
    formatIsoDateInTimeZone(now, timeZone) === dateKey;

  let nowPlaced = false;

  if (isToday && nowTs < events[0]!.startAt.getTime()) {
  ...
  for (let i = 0; i < events.length; i += 1) {
    ...
    if (!shouldShowFreeBlock(gapMs)) {
      if (
        ...
        items.push({
          type: "nowDivider",
          key: `now-small-gap-${ev.id}-${next.id}`,
        });
        nowPlaced = true;
      }
      continue;
    ...
    if (nowTs <= gapStartTs) {
      ...
      appendFreeIfNeeded(
        items,
        beforeMin,
        `free-before-${ev.id}-${next.id}`,
      );

      if (!nowPlaced) {
        items.push({
          type: "nowDivider",
          key: `now-in-gap-${ev.id}-${next.id}`,
        });
        nowPlaced = true;
      }
