    ...
    <section className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        {!isLoading &&
          !errorMessage &&
          items.map((row) => {
            const isConfirmRequired = row.primaryAction === "CONFIRM_OR_DISPUTE";
            return (
              <div key={row.id} className="overflow-hidden rounded-[32px]">
                    ...
                    >
                      <span
                        className={cn(
                          ...
                          isConfirmRequired ? "text-amber-600" : "text-[#6a36d5]",
                        )}
                      >
                ...
                {isConfirmRequired && (
                  <Link
                    to={row.href}
                    className="flex items-center justify-between bg-amber-500 px-4 py-3 text-sm font-bold text-white transition hover:bg-amber-600"
                  >
                    <span>Confirmar serviço realizado</span>
