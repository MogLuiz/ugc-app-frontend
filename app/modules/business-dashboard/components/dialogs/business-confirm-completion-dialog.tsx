...
import type { BusinessPendingConfirmVm } from "../../types";

type Props = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  item: BusinessPendingConfirmVm | null;
};

export function BusinessConfirmCompletionDialog({ open, onOpenChange, item }: Props) {
  const confirmMutation = useConfirmCompletionMutation();
  const titleId = "business-confirm-dialog-title";
    ...
    >
      ...
      >
        ...
        <h2 id={titleId} className="mb-5 text-base font-bold text-[#2c2f30]">
          O creator realizou este serviço?
        </h2>
        ...
        <div className="flex flex-col gap-3">
          <button
            ...
            autoFocus
            className="inline-flex w-full items-center justify-center rounded-[32px] bg-[#6a36d5] py-3 text-sm font-bold text-white shadow-sm transition hover:bg-[#5b2fc4] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#6a36d5]/50 disabled:opacity-60"
            onClick={handleConfirm}
            disabled={confirmMutation.isPending}
          >
            {confirmMutation.isPending ? "Confirmando..." : "Confirmar serviço"}
          </button>
