    ...
    <div className="rounded-lg border border-neutral-200 bg-neutral-50 p-4 space-y-3">
      ...
      <div className="space-y-2 text-sm">
        <div className="flex justify-between">
          ...
          <span className="text-neutral-900">{formatCents(serviceGrossAmountCents, currency)}</span>
        </div>

        {showTransport && (
          <div className="flex justify-between">
            <span className="text-neutral-500">Deslocamento</span>
            <span className="text-neutral-900">{formatCents(transportFeeAmountCents, currency)}</span>
          </div>
        )}

        <div className="flex justify-between">
          <span className="text-neutral-500">Taxa da plataforma</span>
          <span className="text-neutral-900">{formatCents(platformFeeAmountCents, currency)}</span>
        </div>

        <div className="border-t border-neutral-200 pt-2 flex justify-between">
          <span className="font-semibold text-neutral-800">Total</span>
          <span className="font-semibold text-neutral-900">{formatCents(companyTotalAmountCents, currency)}</span>
        </div>
